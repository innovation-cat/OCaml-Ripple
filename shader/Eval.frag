...
void main()
{
	...
	for (i=0; i<n-1; i++)
	{
		...
		float InitT		= ss[base];
		float PosX		= ss[base+1];
		float PosY		= ss[base+2];
		float A			= ss[base+3];
		float Lambda	= ss[base+4];
		float Omega		= ss[base+5];
		float Velocity	= ss[base+6];
		float MaxPhase  = ss[base+7];
